...
from index import check_reachability, check_reachability_with_smart_contract, get_station_number, get_key, find_route_with_smart_contract, find_route_without_smart_contract, calculate_distance, check_node_status_with_smart_contract, check_node_status_without_smart_contract
...
def inconsistency_simulation(stations):
    while not stop_inconsistency_simulation.is_set():
        ...
        station_to_offline.ifconfig(station_to_offline.name + '-wlan0', 'down')
...
def time_estimation(net, sta_start, sta_end):
    while True:
        ...
        # Check node status without smart contract
        status_without_contract_sta_start = check_node_status_without_smart_contract(sta_start, net)
        status_without_contract_sta_end = check_node_status_without_smart_contract(sta_end, net)
        ...
        distances, latest_info = fetch_contract_storage()

        # Check node status with smart contract
        status_with_contract_sta_start = check_node_status_with_smart_contract(sta_start, latest_info)
        status_with_contract_sta_end = check_node_status_with_smart_contract(sta_end, latest_info)

        time_with_contract = time.time() - start_time

        info("\n*** Time Estimation Results ***\n")
        
        info("Node status check time without smart contract for {}: {} seconds\n".format(sta_end, time_without_contract))
        info("Node status: {}\n".format(status_without_contract_sta_end))

        info("Node status check time with smart contract for {}: {} seconds\n".format(sta_end, time_with_contract))
        info("Node status: {}\n".format(status_with_contract_sta_end))
